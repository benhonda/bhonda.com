...
export const shiplogMeta = {
  ...
  previewText: "Target-level search with deduplication, GPU-accelerated HEVC recording with multi-snapshot UI, theme system overhaul, and PostgreSDK transaction primitives.",
  publishedAt: "2026-03-22",
  week: 12,
  year: 2026,
  status: "draft",
  projectTags: ["inspiration-index", "autoscroll-recorder", "bhonda-com", "postgresdk"],
...
export default function Shiplog2026W12() {
  return (
    <ShiplogLayout meta={shiplogMeta}>
      ...
      <Text as="p" variant="body">
        Device presets expanded with <InlineCode>mobile-standard</InlineCode> and <InlineCode>desktop-standard</InlineCode> configurations optimized for T4 VGX GPU at 2x capture scale. Distribution routing UI now supports per-output S3 destinations with InfoPopover help text and duplicate destination controls. Runs can now include notes for context tracking.
      </Text>

      <Text as="p" variant="body">
        Chrome for Testing replaced the previous browser setup with configurable recording parameters including zoom level, UI offsets, and trim seconds. GPU encoder switched from H.264 to HEVC/H.265 (<InlineCode>hevc_nvenc</InlineCode>) with expanded quality flags and p6 preset. Backend migrated from Xvfb to GPU-backed Xorg on AL2023 ECS GPU AMI for hardware-accelerated encoding.
      </Text>
      ...
      <Text as="p" variant="body">
        SEO metadata improvements rolled out across all routes. Theme system overhauled with blocking script to prevent FOUC and Tailwind v4 dark mode integration. Topics browsing launched with <InlineCode>/topics/:topic</InlineCode> route, PostCard component, and tagging taxonomy.
      </Text>
      ...
      <Text as="p" variant="body">
        Upsert operations landed with <InlineCode>sdk.table.upsert({"{ where, create, update }"})</InlineCode> syntax. Transaction support added via <InlineCode>sdk.$transaction()</InlineCode> for atomic multi-operation workflows. Delete operations split into <InlineCode>$softDelete()</InlineCode> and <InlineCode>$hardDelete()</InlineCode> methods for explicit control.
      </Text>

      <Text as="p" variant="body">
        Stale file cleanup now prompts before deletion with <InlineCode>--force</InlineCode> flag for non-interactive environments. Pull operation automatically detects and removes orphaned output files.
      </Text>
    </ShiplogLayout>
  );
}
